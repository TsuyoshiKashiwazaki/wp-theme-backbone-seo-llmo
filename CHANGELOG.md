...
### 追加
- フロントページ設定の一覧表示セクションに「子階層の投稿タイプの記事も含める」オプションを追加
  - 表示対象が「投稿タイプ」のときに表示されるチェックボックス。ONにすると、選択した投稿タイプの子階層・孫階層の投稿タイプの記事もまとめて表示する
  - 親子関係は Kashiwazaki SEO Custom Post Types の「親ディレクトリ」設定から判定し、プラグインが無い場合はパーマリンクの階層で判定する
  - 表示件数・並び順はまとめた記事全体に適用し、「一覧を表示」リンクは選択した投稿タイプの一覧を指す
  - 既定値はOFFのため、更新しても従来の挙動は変わらない

## [1.2.3] - 2026-09-21

### 追加
- カスタマイザーに「インデックス設定（noindex）」セクションを追加
  - タグ・カテゴリの個別アーカイブを「noindexにしない／記事数がしきい値以下のものだけ／すべて」から選択可能
  - しきい値は数値で指定でき、記事数が増えれば自動的にインデックス対象へ戻る
  - タグ一覧（/tag/）・カテゴリ一覧（/category/）、著者・日付・2ページ目以降も個別に設定可能
  - 既定値はすべて「noindexにしない」のため、更新しても従来の挙動は変わらない

### セキュリティ修正
- taxonomy_root クエリ変数を外部入力から操作できる問題を修正
  - 公開クエリ変数として登録されていたため、任意のURLに付与するだけでタグ一覧テンプレートが描画され、正規ページとは異なる内容・タイトル・canonical が出力されていた
  - レイアウトとbodyクラスも同様に切り替わっていた
  - リライト規則が生成した値のみを信頼する判定に統一し、GET・POST いずれの経路からも操作できないようにした

### 修正
- アーカイブ系ページで PHP 警告が発生する問題を修正
  - get_queried_object() の戻り値を型で判定するよう変更（メタディスクリプション・メタキーワードの生成箇所）
  - post_type クエリ変数が配列の場合に文字列として連結していた問題を修正（レイアウト判定・bodyクラス生成）
  - get_the_archive_title() が null を返す場合に preg_replace へ渡していた問題を修正
- robots メタタグで follow を明示出力しないよう変更（他の拡張が nofollow を指定した場合の矛盾を防止）

## [1.2.2] - 2026-09-14

### 追加
- カスタマイザーに「追加タグ」セクションを追加 (inc/customizer/custom-tags-settings.php, inc/custom-tags-output.php)
  - メタタグ・計測タグなどを `<head>` 内・`<body>` 直後・`</body>` 直前にそのまま出力できる
  - 「全体共通」と「トップページのみ」の 2 種類。`unfiltered_html` 権限を持つユーザーだけが編集でき、カスタマイザーのプレビューでは出力しない

### セキュリティ修正
- パスワード保護された投稿・固定ページで、抜粋や本文の冒頭がメタディスクリプションに出力される問題を修正 (inc/utilities/core-utilities.php)
  - 本文から生成する場合はショートコードとブロックの記法を取り除いてから切り出すように変更
- カスタマイザーで選択した投稿を後から非公開・下書きにしても、トップページの個別セクション・ピックアップ・「既存のページを使用」モード・投稿ページに表示され続ける問題を修正 (front-page.php, home.php, template-parts/sections/individual-section.php, template-parts/sections/pickup.php)
  - 公開投稿は全員に、非公開投稿は閲覧権限を持つユーザーにだけ表示する判定 `backbone_is_post_displayable()` を追加 (inc/utilities/core-utilities.php)
  - メインビジュアルの説明文ソースと投稿ページで、パスワード保護された本文を出力しないように変更 (template-parts/sections/hero.php, home.php)
- テーマ設定のバックアップの復元とインポートを `unfiltered_html` 権限を持つユーザーに限定 (inc/admin-pages.php)
  - バックアップの値はカスタマイザーのサニタイズを通らずに書き込まれ、一部は `<style>` 内などにそのまま出力されるため

### 修正
- 検索と投稿タイプの絞り込み (`?s=...&post_type=...`) を併用すると、検索語と無関係な記事が一覧に表示される問題を修正 (functions.php)
  - 並び順を補正する再クエリが検索語・カテゴリ・著者などの条件を引き継いでいなかった。検索時は補正しないようにし、再クエリはメインクエリの条件を引き継いで件数も更新する
- サブディレクトリ設定のパス判定で、`/blog` が `/blog-creator/` や `/news/blog-creator/` にも一致し、別のページにサイト名・ロゴ・デザインが適用される問題を修正 (inc/customizer/subdirectory-logos.php, inc/customizer/subdirectory-design-settings.php)
  - ホーム URL を基準にしたパスで、`/` の区切り単位で前方一致を判定するように統一
- 固定ページの REST API で抜粋 (excerpt) を文字列で上書きし、形式が投稿と食い違っていた問題を修正 (functions.php)
  - 独自の `register_rest_field('page', 'excerpt')` を削除し、WordPress 本体の抜粋フィールドを使用
- テーマ有効化時に、設定済みのパーマリンク構造を `/%category%/%postname%/` に上書きする問題を修正 (inc/theme-setup.php)
  - パーマリンク構造が未設定 (基本) の場合だけ設定する
- テーマ設定のリセット後、またはサブディレクトリ設定を含まないバックアップの復元後に、サブディレクトリ設定が次のアクセスで復活する問題を修正 (inc/admin-pages.php)
- サブディレクトリ設定の削除ボタンを連続で押すと、選択していない設定まで削除されることがある問題を修正 (inc/customizer/subdirectory-logos.php)
  - 存在しない番号の削除要求を拒否し、送信中は再送しないように変更
- ヘッダーの検索ボタンがサブメニュー内にも挿入される問題を修正 (header.php)
- 記事が 0 件のカスタム投稿タイプのアーカイブで、メタディスクリプションが「Archive for」だけになり PHP の警告が出る問題を修正 (inc/utilities/core-utilities.php)
  - 投稿タイプ名を表示中の記事ではなくクエリから取得するように変更
  - カテゴリや投稿タイプで絞り込んだ検索では、アーカイブの文言ではなく検索結果の文言を出力するように変更

### その他
- LICENSE を GPL v2 の全文に更新
- .gitignore に開発ツールの設定ファイルを追加

### ドキュメント
- docs: 追加タグの説明を追加し、パーマリンク・サブディレクトリの判定・非公開投稿の扱いの記述を実装に合わせて更新
- docs: メタディスクリプションの生成ルール表を実際の出力に合わせて修正（フォールバックの定型文は英語、固定ページをホームページに指定した場合のルール、カスタム投稿タイプ・検索結果の行を追加）
- README.md / readme.txt: メタディスクリプションの説明に残っていた旧仕様（「本文の最初の25語」）を「本文の先頭160文字」に修正

## [1.2.1] - 2026-08-02

### 修正
- メタディスクリプションが日本語で25文字に切り詰められる問題を修正 (inc/utilities/core-utilities.php)
  - `wp_trim_words()` は語数で数えるため、単語の区切りがない日本語では25文字で切れていた
  - 抜粋が未設定の投稿・固定ページで、説明文が文の途中で終わる状態になっていた
- メタディスクリプションの末尾に不要な「...」が付く問題を修正 (inc/utilities/core-utilities.php)
  - 文字数上限の判定に `strlen()`（バイト数）を使い、切り出しに `mb_substr()`（文字数）を使っていたため単位が不一致だった
  - 日本語では54文字を超えた時点で必ず分岐に入り、実際には切り詰めていないのに「...」だけが付与されていた

### ドキュメント
- docs: メタディスクリプションの生成ルールの記述を実装に合わせて修正（「本文の先頭25語」→「本文の先頭160文字」）
...
### 修正
- タグ・カテゴリ一覧ページ（`/tag/`、`/category/`）のカウント表示が実際の記事数と一致しない問題を修正
  - `$term->count`（DBキャッシュ値）の代わりに、全公開投稿タイプの公開記事数をSQLで一括取得
  - アーカイブページに表示される記事数と完全に一致するカウントを表示

### 技術的変更
- 変更: `taxonomy-root.php` - タームカウント取得ロジックを実クエリベースに変更
...
### 追加
- ページトップへ戻るボタン機能を追加（サイズ・位置のカスタマイズ対応）
  - カスタマイザーでボタンサイズ（小/中/大）を選択可能
  - ボタン位置（左/中央/右）を選択可能
  - 表示/非表示の切り替えに対応

### 改善
- カスタマイザーUI改善: スティッキーヘッダー無効時に関連オプション（透明度・自動非表示）を非表示化
  - `active_callback` を追加してスティッキーヘッダー設定との連動を実現

### 修正
- フロントページ説明文の表示条件を修正
  - タイトルなしで説明文のみ設定されている場合でも正しく表示されるように

### 技術的変更
- 変更: `inc/customizer/layout-settings.php` - スティッキーヘッダー関連オプションに `active_callback` 追加
- 変更: `inc/theme-setup.php` - ページトップボタンのインラインCSS出力を追加
- 変更: `template-parts/sections/hero.php` - 説明文表示条件の修正
- 変更: `css/components-footer.css` - ページトップボタンのベーススタイル追加
- 変更: `js/theme.js` - ページトップボタンの表示/非表示ロジック追加

## [1.0.36] - 2026-01-07

### 修正
- 著者アーカイブページのh1見出しで `<span class="vcard">` タグが文字として表示される問題を修正

## [1.0.35] - 2026-01-04

### 追加
- フロントページ設定の「説明文のソース」で下書き・非公開の記事も選択・表示可能に
- ソースページ選択時に【下書き】【非公開】ラベルを表示

## [1.0.34] - 2026-01-03

### 追加
- 検索可能セレクトコントロール（選択表示と検索入力を分離したUI）
- ソースページのアイキャッチ画像をメインビジュアルに使用するオプション
- TITLEを「投稿タイトル | サイト名」形式にするオプション
- ブロックアセットの汎用的な読み込み機能（has_block対応で外部投稿のブロックも正しく表示）

### 変更
- カスタムHTMLブロックのスクリプト出力を許可（Chart.js等のJSが動作するように）
- 全44カラーテーマのヘッダー/フッターリンク色を調整

### 修正
- 検索可能セレクトで選択値と検索入力が混在する問題
- レイアウトCSSの不要なテキストカラー指定を削除

## [1.0.33] - 2025-12-25

### 追加
- バックアップのエクスポート/インポート機能（JSONファイルで他サイトに持ち出し可能）
- テーマ設定のリセット機能
- バックアップにメタ情報追加（テーマバージョン、サイトURL、親テーマ情報）
- 復元時のデータ検証（無効なメディアID/投稿ID/URLを警告）
- バージョン互換性チェック（親テーマのバージョンで判定）

### 変更
- ページ名称を「テーマ診断・バックアップ」→「テーマ設定のバックアップと復元」に変更
- メニュー名を「診断・バックアップ」→「バックアップと復元」に変更
- バックアップ一覧表示を簡略化（親テーマバージョン + サイトURLのみ）

### 修正
- 子テーマから別テーマへの切り替え時に自動バックアップが動作しない問題
- エクスポート時にHTMLが出力される問題
